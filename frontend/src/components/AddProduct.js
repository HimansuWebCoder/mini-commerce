...
async function addProducts() {
    ...
    setImageUrl(cloudData.secure_url);
     ...
     fetch("http://localhost:4000/products", {
          method: "post",
          headers: {"Content-Type": "application/json"},
          body: JSON.stringify({ productName, category:productCategory, subcategory:productSubCategory, productOriginalPrice, discount, productImg: cloudData.secure_url, productPrice, productDescription }),
          credentials: "include"
     })
     .then(res => res.json())
     .then(() => {
         ...
         <div>
            ...
            <div  style={{backgroundImage: `url("./images/bg.png")`}} className="fixed top-0 flex justify-center items-center backdrop-filter backdrop-blur-[2px] left-0 z-20 w-full h-screen">
             ...
             <div className="max-w-[40rem] m-auto p-4 rounded-2xl grid grid-cols-2 relative gap-4  bg-white shadow-2xl">
                 <div onClick={() => setIsShowModal(isShow)} className="absolute cursor-pointer top-2 right-2 p-1 rounded-full aspect-[1/1]">
                    ...
                    <div className="flex flex-col w-full h-fit  justify-center gap-2 items-start">
                         <label className="font-inter text-xl text-red-400 font-medium" htmlFor="select-category">Sub Category</label>
                        <select className="w-full rounded-lg h-fit p-2 focus:outline-red-400" onChange={addProductSubCategory} value={productSubCategory} id="select-category" name="select-category">
                        {
